...
- [x] on the homepage the rightrail and footer with the buttons should be static and only the inner section with the segments should scroll
...
- [x] while a segment is being generated the text often updates unnecessarily, sometimes even temporarily going blank, come up with ways to mitigate this
    - [x] possibly incorporate confidence into the emit segment update logic (with a setting for it)
    - [x] don't emit a segment if the text changed too drastically
...
- [ ] installer should also install CUDA on windows
